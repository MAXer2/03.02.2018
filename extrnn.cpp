...
void risovat_prepyatstvie(int x_prep,int y_prep, HDC alisa);
void risovat_prepyatstvie2(int x_prep2,int y_prep2,HDC bazilla);
...
int main() {
    ...
    HDC alisa = txLoadImage("lisalevo.bmp");

    HDC bazilla = txLoadImage("kotbazilla.bmp");

    HDC menu = txLoadImage("menushka.bmp");
    HDC kartinka;
    ...
    while (game_over == false) {
       ...
       //Собрал монету
        if (abs (x - moneta_x) + abs (y - moneta_y) < 150 &&
            monetok_s_soboi < 1 &&
            monetok_v_kopilke <5) {
        ...
        risovat_prepyatstvie (x_prep,  y_prep,  alisa);
        risovat_prepyatstvie2(x_prep2, y_prep2, bazilla);

        if (abs(x- x_prep) < 100 && abs(y- y_prep) < 100) {
            game_over = true;
        }
        if (abs(x- x_prep2) < 100 && abs(y- y_prep2) < 100) {
            game_over = true;
        }
...
void risovat_prepyatstvie(int x_prep,int y_prep,HDC alisa){
  txTransparentBlt (txDC(), x_prep,y_prep , 194, 278, alisa, 0, 0, TX_WHITE);
}
void risovat_prepyatstvie2(int x_prep2,int y_prep2, HDC bazilla){


    txTransparentBlt (txDC(), x_prep2,y_prep2 , 120, 212, bazilla, 0, 0, TX_WHITE);
}
void risovatMenu(HDC menu){
